...
exports.run = async (client, message, args) => {
  if (!message.member.roles.cache.has("774301169367253033"))
    return message.channel.send(
      `${message.author}, \<@&774301169367253033>\ komutunu kullanmak için yeterli yetkiye sahip değilsin.`
    );

  let user = message.mentions.members.first();

  if (!user) return message.channel.send("Bir Üye Etiketleyin");
...
*/
  ...
  user.roles.add("774299684227252225");
  user.roles.remove("774299395255435285");
...
**❯ Cinsiyet** : Kadın
  ...
  user.roles.remove("762306275916644372");
    ...
    const s = new Discord.MessageEmbed()
      ...
      .setDescription(
        `
❯ 7/24 Code Sunucusun'a Hoş Geldiniz.
❯  Aramıza katıldı.
